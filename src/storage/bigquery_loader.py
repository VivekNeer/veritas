...
class SQLiteBackend:
    ...
    def __init__(self, db_path: str = "local_warehouse.db"):
        ...
        self._ensure_tables_exist = set()

    def _ensure_table(self, table: str, sample_row: dict) -> None:
        if table in self._ensure_tables_exist:
            return
        cols = ", ".join(f'"{k}"' for k in sample_row.keys())
        placeholders = ", ".join(f'"{k}" TEXT' for k in sample_row.keys() if k != "id")
        self.conn.execute(
            f'CREATE TABLE IF NOT EXISTS "{table}" (id TEXT PRIMARY KEY, {placeholders})'
        )
        self.conn.commit()
        self._ensure_tables_exist.add(table)

    def load_records(self, rows: list[dict]) -> int:
        if not rows:
            return 0
        self._ensure_table("standardised_records", rows[0])
    ...
    def load_dead_letters(self, rows: list[dict]) -> int:
        if not rows:
            return 0
        sample = {**rows[0], "id": None}
        self._ensure_table("dead_letter", sample)
        cols = ["id"] + [c for c in sample.keys() if c != "id"]
        placeholders = ", ".join("?" for _ in cols)
        col_list = ", ".join(f'"{c}"' for c in cols)
        sql = f'INSERT OR REPLACE INTO "dead_letter" ({col_list}) VALUES ({placeholders})'
        values = []
        for r in rows:
            rid = hashlib.sha256(json.dumps(r, sort_keys=True, default=str).encode()).hexdigest()[:32]
            values.append([rid] + [r.get(c) for c in cols if c != "id"])
        self.conn.executemany(sql, values)
        self.conn.commit()
        return len(rows)
...
class BigQueryBackend:
    ...
    def _ensure_table(self, table: str, sample_row: dict) -> None:
        ...
        try:
            ...
            pass
        schema = [
            self.bigquery.SchemaField(k, "FLOAT64" if isinstance(v, float) else "STRING", mode="NULLABLE")
    ...
    def load_dead_letters(self, rows: list[dict]) -> int:
        if not rows:
            return 0
        self._ensure_table("dead_letter", rows[0])
        job_config = self.bigquery.LoadJobConfig(
            source_format=self.bigquery.SourceFormat.NEWLINE_DELIMITED_JSON,
            write_disposition=self.bigquery.WriteDisposition.WRITE_APPEND,
        )
        job = self.client.load_table_from_json(rows, self._table_ref("dead_letter"), job_config=job_config)
        job.result()
        return len(rows)
